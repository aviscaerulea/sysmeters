...
| メニュー | タスクトレイ右クリック：常に最前面に表示（トグル）、設定ファイルを開く、ログを開く、終了 |
...
| 要素 | 表示形式 |
|---|---|
| グローバル IP | テキスト（例：`203.0.113.89`）、取得失敗時は `NO INTERNET📵` |
...
ログ出力対象：起動・終了・ウィンドウ作成失敗、各コレクタの初期化成功/失敗、Claude API の HTTP/JSON エラー。
