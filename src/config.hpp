...
struct AppConfig {
    ...
    // トッププロセス表示の表示条件（[topproc] show_pct、CPU/GPU 共通）
    // 大パーセンテージ（面グラフ左端の使用率）がこの値以上のときのみトッププロセス名を表示する。
    // 0 でこの条件を無効化。（min_share_pct 条件は独立に効くため常時表示にはならない）
    // 低負荷時は「何が一番使っているか」の情報価値が薄いため隠す意図。
    // 表示のみの条件で、トレイトグル ON 中の収集は継続する。（閾値を跨いだ瞬間に即表示するため）
    // 表示中は min_share_pct と共に閾値を 0.8 倍に緩めて判定する。（境界付近の点滅防止）
    float topproc_show_pct = 50.f;
    // トッププロセスの占有割合の下限（[topproc] min_share_pct、CPU/GPU 共通）
    // トッププロセスの使用率が「大パーセンテージ × この値 / 100」以上のときのみ表示する。
    // 例：全体 90% で 50 なら、トップが 45% 以上のときだけ表示する。
    // 全体が高負荷でも特定プロセスに負荷が偏っていなければ「何が特に高負荷か」の情報に
    // ならないため隠す意図。0 で無効。（占有割合による絞り込みをしない）
    // 既知の制約：GPU 側はトップ値（バケット最大）と大パーセンテージ（NVML 全体稼働率）の
    // 出所が異なり、3D と Compute を併用するプロセスでは割合が実態より低く出て
    // 非表示に倒れることがある。値を下げると緩和できる。
    // また GPU のトップ値は約 2.7 秒間隔の更新のため、負荷急増の遷移中は古いトップ値との
    // 比較で占有割合が一時的に低く出て、最大 2.7 秒間表示が消えることがある。（自動復帰する。
    // linger_sec > 0 の間はこの消失が残像表示で覆われ、既定値 5 秒では見た目上は消えない）
    float topproc_min_share_pct = 50.f;
    // トッププロセスの残像表示の長さ（[topproc] linger_sec、秒、CPU/GPU 共通）
    // 表示条件を失った後も、最後に表示した名前と % をこの秒数だけ凍結して薄く表示する。
    // 一瞬のスパイクで表示を見逃すのを防ぐ意図。0 で無効。（条件喪失で即非表示）
    // サニティチェックで 0〜600 にクランプする。
    // 対象はデータ由来の消失のみで、トレイトグル OFF・セクション非表示・GPU N/A では
    // 残像を出さない。（renderer 側の残像状態を即時消去する）
    int topproc_linger_sec = 5;
    ...
    // 現在の使用率が均等消費ペース位置（緑線）を超過している間も判定しない。
    // 警告色（黄・赤）と暗青背景が同居して状態を判断できなくなるのを防ぐため
    ...
    // 5h リセット通知の下限使用率（%）。リセット時刻を通過したとき、直前の 5h 使用率が
    // この値以下なら通知しない（未使用のウィンドウのリセットは知らせる価値が無い）。
    // 通知方法（Toast / 通知音 / 通知しない）はトレイメニューで選び、レジストリに保持する。メイン/サブ共通。
    // サニティチェックで 0〜100 にクランプする
    float claude_reset_notify_min_pct = 1.f;
    // nudge 実行時のコマンドライン。
