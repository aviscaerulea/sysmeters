...
struct AppConfig {
    ...
    // 5h/7d バーの未使用部分の背景を暗紫にする。
    // 表示のみで警告音・Toast は発しない。メイン/サブ共通
    bool  claude_underuse_enable   = true;
    // 予測到達率の目標値（%）。追い上げ外挿の予測がこの値未満なら警告表示。
    // サニティチェックで 0〜100 にクランプする
    float claude_underuse_warn_pct = 98.f;
    // ペース算出の観測ウィンドウ幅（分）。この時間ぶんの観測サンプルの端点差分を
    // 消費ペースとして外挿に使う。サニティチェックで 30〜1440 にクランプする
    int   claude_underuse_pace_window_min = 150;
    // 判定開始までの猶予時間（分）。ウィンドウ開始からこの時間が経過するまで判定しない。
    // ウィンドウ序盤は調整しながらの使用が普通で、警告して使用を促すのは早計なため。
    // サニティチェックで 5h は 0〜300、7d は 0〜10080（各ウィンドウ長）にクランプする
    int   claude_underuse_grace_5h_min = 120;
    int   claude_underuse_grace_7d_min = 2880;
    // 検知除外のしきい値（分）。使用率が「この時間ぶんの均等消費」に相当する率以下の間は
    // 判定しない。低使用時に常時警告となるのを防ぐ。
    // サニティチェックで 5h は 0〜300、7d は 0〜10080（各ウィンドウ長）にクランプする
    int   claude_underuse_ignore_5h_min = 60;
    int   claude_underuse_ignore_7d_min = 1440;
    // 均等ペース遅れしきい値（分）。使用率の均等消費位置（ウィンドウ経過率）からの遅れが
    // 「この時間ぶんの均等消費」に相当する率未満の間は判定しない。
    // 概ね均等消費に追従できている・先行している状態での過剰警告を防ぐ。
    // 0 は遅れゼロ以上（先行していない）で判定する意味になる。
    // サニティチェックで 5h は 0〜300、7d は 0〜10080（各ウィンドウ長）にクランプする
    int   claude_underuse_lag_5h_min = 60;
    int   claude_underuse_lag_7d_min = 720;
    // nudge 実行時のコマンドライン。
