...
// 数値でない要素はスキップする。ここでは鮮度チェックをしない。古いサンプルは呼び出し側の
// push_and_trim が落とすためだ。未来時刻のサンプルは、描画側の濃色オーバーレイが
// 「N 分以上前」を条件に起点を探すため起点に選ばれず、無害だ。
...
void ClaudeCollector::do_fetch() {
    ...
    // 始まっていない間隙を検知したら nudge_cmd を起動する。発火は間隙につき初回を含め
    // NUDGE_MAX_ATTEMPTS 回までで、空振りして間隙が続くときのみ claim_nudge が再発火を許可する。
