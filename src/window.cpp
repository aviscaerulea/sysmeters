...
void AppWindow::show_context_menu() {
    ...
    // 抑制機能自体が OFF の間は効果を持たないため、ドライブ別トグルと同様に MF_GRAYED で無効表示する
    {
        HMENU aa_menu = CreatePopupMenu();
        const UINT gray = fullscreen_mute_ ? 0 : MF_GRAYED;
        AppendMenuW(aa_menu, MF_STRING | gray | (always_alert_cpu_       ? MF_CHECKED : 0),
                    IDM_ALWAYS_ALERT_CPU,       L"CPU 使用率");
        AppendMenuW(aa_menu, MF_STRING | gray | (always_alert_temp_cpu_  ? MF_CHECKED : 0),
                    IDM_ALWAYS_ALERT_TEMP_CPU,  L"CPU 温度");
        AppendMenuW(aa_menu, MF_STRING | gray | (always_alert_gpu_       ? MF_CHECKED : 0),
                    IDM_ALWAYS_ALERT_GPU,       L"GPU 使用率");
        AppendMenuW(aa_menu, MF_STRING | gray | (always_alert_temp_gpu_  ? MF_CHECKED : 0),
                    IDM_ALWAYS_ALERT_TEMP_GPU,  L"GPU 温度");
        AppendMenuW(aa_menu, MF_STRING | gray | (always_alert_temp_disk_ ? MF_CHECKED : 0),
                    IDM_ALWAYS_ALERT_TEMP_DISK, L"ディスク温度");
        AppendMenuW(menu, MF_POPUP | MF_STRING, reinterpret_cast<UINT_PTR>(aa_menu),
                    L"常に警告通知を有効にする");
    }
...
static constexpr LPCWSTR REG_ALWAYS_ALERT_GPU       = L"AlwaysAlert_GPU";
static constexpr LPCWSTR REG_ALWAYS_ALERT_TEMP_GPU  = L"AlwaysAlert_TempGPU";
static constexpr LPCWSTR REG_ALWAYS_ALERT_TEMP_DISK = L"AlwaysAlert_TempDisk";
...
void AppWindow::load_always_alert() {
    ...
    always_alert_gpu_       = load_reg_bool(REG_ALWAYS_ALERT_GPU,       DEF_ALWAYS_ALERT_GPU);
    always_alert_temp_gpu_  = load_reg_bool(REG_ALWAYS_ALERT_TEMP_GPU,  DEF_ALWAYS_ALERT_TEMP_GPU);
    always_alert_temp_disk_ = load_reg_bool(REG_ALWAYS_ALERT_TEMP_DISK, DEF_ALWAYS_ALERT_TEMP_DISK);
}

void AppWindow::save_always_alert() {
    save_reg_bool(REG_ALWAYS_ALERT_CPU,       always_alert_cpu_);
    save_reg_bool(REG_ALWAYS_ALERT_TEMP_CPU,  always_alert_temp_cpu_);
    save_reg_bool(REG_ALWAYS_ALERT_GPU,       always_alert_gpu_);
    save_reg_bool(REG_ALWAYS_ALERT_TEMP_GPU,  always_alert_temp_gpu_);
    save_reg_bool(REG_ALWAYS_ALERT_TEMP_DISK, always_alert_temp_disk_);
}

// 「常に警告通知」フラグを AlertManager::Id のビットマスクに変換する
// ディスク温度は 1 トグルで全ドライブ扱いのため TEMP_NVME_0..7 の全添字に展開する
// （未検出の添字はビットが立っていても check() が発火させないため無害）
uint32_t AppWindow::always_alert_mask() const {
    uint32_t mask = 0;
    if (always_alert_cpu_)      mask |= 1u << AlertManager::CPU;
    if (always_alert_temp_cpu_) mask |= 1u << AlertManager::TEMP_CPU;
    if (always_alert_gpu_)      mask |= 1u << AlertManager::GPU;
    if (always_alert_temp_gpu_) mask |= 1u << AlertManager::TEMP_GPU;
    if (always_alert_temp_disk_)
        for (int i = 0; i < kMaxDiskDrives; ++i)
            mask |= 1u << (AlertManager::TEMP_NVME_0 + i);
    return mask;
}
...
LRESULT AppWindow::handle_message(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_TIMER:
        ...
        if (alert_) {
            ...
            // フルスクリーン抑制の開始/解除をタイトルバーに反映する（変化時のみ SetWindowText）
            if (muted != fullscreen_silent_) {
                fullscreen_silent_ = muted;
                SetWindowTextW(hwnd_, muted ? L"sysmeters (fullscreen:silent)" : L"sysmeters");
            }
            // 「常に警告通知」の例外項目は抑制中でも警告音（check 内）と Toast を通す
            const uint32_t always_mask = always_alert_mask();
    ...
    case WM_COMMAND: {
        ...
        case IDM_ALWAYS_ALERT_TEMP_CPU:  always_alert_temp_cpu_  = !always_alert_temp_cpu_;  save_always_alert(); break;
        case IDM_ALWAYS_ALERT_GPU:       always_alert_gpu_       = !always_alert_gpu_;       save_always_alert(); break;
        case IDM_ALWAYS_ALERT_TEMP_GPU:  always_alert_temp_gpu_  = !always_alert_temp_gpu_;  save_always_alert(); break;
        case IDM_ALWAYS_ALERT_TEMP_DISK: always_alert_temp_disk_ = !always_alert_temp_disk_; save_always_alert(); break;
        case IDM_STARTUP:
