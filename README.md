...
- **トッププロセス表示**：CPU / GPU の面グラフ内に、使用率トップのプロセス名と使用率を `chrome 28%` の形式で表示する。同名 exe（Chrome の複数プロセス等）は合算し、`.exe` を除いた名前で表示する。CPU は `NtQuerySystemInformation` のスナップショット差分（0.9 秒間隔、システム全体を 100% とする尺度）、GPU は PDH の `\GPU Engine(*)\Utilization Percentage` の 3D/Compute 系エンジンを同名 exe・アダプタ・エンジン種別の単位で合算し、その最大値を採って求める。（約 2.7 秒間隔）タスクトレイメニューの「トッププロセス表示」で ON/OFF を切り替えられ、OFF の間は収集自体を停止する。設定はレジストリに保持され、再起動後も維持される。デフォルト ON。大パーセンテージが `sysmeters.toml` の `[topproc] show_pct`（デフォルト 10%、0 で常時表示）未満の間は表示を隠す
