...
- **Claude Code**：5h / 7d レートリミット使用率（横バー）+ リセット時刻、セッション数。メイン / サブの 2 アカウントを縦並びで同時表示できる（サブは `sysmeters.toml` の `[claude_sub] enable = true` と `config_dir` 指定で有効化）。横バー上の緑の縦線は均等消費ペースマーカー（リセットまでの残り時間で均等に消費した場合の理想消費位置）。5h バーには直近 N 分間で増えた使用率を濃色（RAM の WSL オーバーレイと同色）で重ね塗りし、消費ペースを視覚化する（`sysmeters.toml` の `[claude] delta_window_min`、デフォルト 5 分、0 で無効）。直近 `[claude] underuse_pace_window_min`（デフォルト 150 分）の観測傾き（端点差分）のペースで残り時間を消費し続けても使用率が目標値（`[claude] underuse_warn_pct`、デフォルト 98%）へ届かない＝挽回不能を検知すると、バーの未使用部分の背景が暗紫色に変わる。（ウィンドウ開始から猶予時間が経過するまで判定しない。5h は `underuse_grace_5h_min` デフォルト 120 分、7d は `underuse_grace_7d_min` デフォルト 2880 分＝48 時間。ペース観測が 30 分に満たない起動直後・ウィンドウ切替直後も判定しない。使用率が 5h では 60 分ぶん＝20%、7d では 24 時間ぶん≒14.3% 以下の低使用時は判定対象外。`underuse_ignore_5h_min` / `underuse_ignore_7d_min` で調整可能。均等消費位置（緑線）からの遅れが 5h では 60 分ぶん＝20%、7d では 12 時間ぶん≒7.1% に満たない間も、概ね均等消費に追従できているため判定しない。`underuse_lag_5h_min` / `underuse_lag_7d_min` で調整可能。7d の使用量が均等消費位置を超過している間は、7d 枠逼迫時に 5h の消費を促さないため 5h を判定しない。`underuse_enable = false` で機能無効化）5h バーには PT 平日 5:00-10:59 を暗赤色で示すピーク時間帯表示があるが、2026-05 の Anthropic によるレート制限撤廃に伴いデフォルト OFF（`sysmeters.toml` の `[claude] show_peak_bar = true` で有効化可能）。毎時 0 分に最新データを強制取得する。緑の理想位置からの超過率が閾値を超えると赤文字・警告音・Toast 通知をアカウント別に発する。使用率が 100% に達した場合も赤文字で表示する。7d バーが理想位置超過の警告（黄・赤文字）中は、バー左端に警告解除までの残り時間を黒字で表示する（60 分以内は `-20m` のように分単位、60 分超は `-1.1h` のように時間単位）。塗り幅が文字の表示に足りない低使用時は表示しない。セッション数はアカウントごとに分けてカウントする（`claude.exe` のコマンドラインから判別）。Sessions ラベルの左には直近の Usage API 取得時刻（`H:MM` 形式）を同色・同サイズで表示し、表示データの鮮度を確認できる
