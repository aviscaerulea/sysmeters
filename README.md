...
- 5h バーの右側（7d リセット日時の月数字と右端を揃えた位置）には、7d リセットまでに実行できる 5h ターンの残数（進行中のターンを含む）を表示
...
- Sessions ラベルの左には直近の Usage API 取得時刻（`HH:MM` 形式、時はゼロ埋めなし）を同色・同サイズで表示し、表示データの鮮度を確認できる
...
| `[claude]` | Claude Code セクションの表示調整、nudge、5h リセット通知 |
