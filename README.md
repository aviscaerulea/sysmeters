...
直近の消費ペースのままでは目標使用率に届かない＝挽回不能と判断すると、バーの未使用部分の背景を暗紫色に変える。表示のみで警告音・Toast は発しない。次のすべてを満たしたときのみ発生する。（`sysmeters.toml` の `[claude] underuse_enable = false` で機能自体を無効化できる）

- 直近 `underuse_pace_window_min`（デフォルト 150 分）の観測傾きで残り時間を消費し続けても、予測到達率が `underuse_warn_pct`（デフォルト 98%）に届かない（観測スパンが 30 分に満たない起動直後・ウィンドウ切替直後は判定しない）
- ウィンドウ開始から `underuse_grace_5h_min` / `underuse_grace_7d_min`（デフォルト 120 分 / 2880 分＝48 時間）以上経過している
- 使用率が `underuse_ignore_5h_min` / `underuse_ignore_7d_min`（デフォルト 60 分＝20% / 1440 分≒14.3%）に相当する率を超えている
- 均等消費ペース位置（緑線）からの遅れが `underuse_lag_5h_min` / `underuse_lag_7d_min`（デフォルト 60 分＝20% / 720 分＝12 時間≒7.1%）に相当する率以上ある
- （5h のみ）7d の使用量が均等消費ペース位置を超過していない（7d 枠逼迫時は 5h の消費を促さないため判定しない）
