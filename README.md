...
- 直近 `delta_window_7d_min` 分の平均消費ペース（濃色オーバーレイと同じ観測ウィンドウ）で残り時間を消費し続けても、予測到達率が `underuse_warn_pct`（デフォルト 98%）に届かない

ペースが推定できない間（初回起動でキャッシュ未生成のとき、リセット直後で履歴がウィンドウ幅に満たないとき、直近の増加がないとき）は判定しない。7d の履歴は `$TEMP` にファイル保存されるため、2 回目以降の起動では直近 12h 分の履歴を引き継いで判定を継続できる。
