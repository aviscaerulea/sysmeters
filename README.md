...
- **Claude Code**：5h / 7d レートリミット使用率（横バー）+ リセット時刻、セッション数。横バー上の緑の縦線は均等消費ペースマーカー（リセットまでの残り時間で均等に消費した場合の理想消費位置）。毎時 0 分に最新データを強制取得する。緑の理想位置からの超過率が閾値を超えると赤文字・警告音・Toast 通知を発する
