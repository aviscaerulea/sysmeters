...
- **CPU**：全体使用率（面グラフ）+ ハードフォールト（重畳面グラフ）+ 論理コア別使用率（縦バー、実際の論理コア数分）+ 温度（横バー）+ CPU 使用率トップのプロセス名（面グラフ内）+ システム統計（Proc/Thread/Handle、閾値超過で赤文字）
- **GPU**：使用率（面グラフ）+ 温度（横バー）+ GPU 使用率トップのプロセス名（面グラフ内）、NVIDIA NVML 経由
- **RAM**：使用率（横バー）+ 使用量/総量
...
- **Claude Code**：5h / 7d レートリミット使用率（横バー）+ リセット時刻、セッション数。メイン / サブの 2 アカウントを縦並びで同時表示できる（サブは `sysmeters.toml` の `[claude_sub] enable = true` と `config_dir` 指定で有効化）。横バー上の緑の縦線は均等消費ペースマーカー（リセットまでの残り時間で均等に消費した場合の理想消費位置）。5h バーの右側（7d リセット日時の月数字と桁が揃う位置）には、7d リセットまでに実行できる 5h ターンの残数（進行中のターンを含む）を表示する。（`sysmeters.toml` の `[claude] turns_show_from` で表示開始値を調整可能。デフォルト 9、0 で無効）5h / 7d バーには直近 N 分間で増えた使用率を濃色（RAM の WSL オーバーレイと同色）で重ね塗りし、消費ペースを視覚化する（`sysmeters.toml` の `[claude] delta_window_min` / `delta_window_7d_min`、デフォルト 5 分 / 720 分＝12 時間、0 で無効）。7d バーの直下には上位モデル（Fable 等）専用 7d 枠の消費率を示すグレーのミニバーを表示する。（Usage API が専用枠を返すアカウントのみ。色は `sysmeters.toml` の `[color] claude_scoped_bar`、縦幅は `[claude] scoped_bar_px`、デフォルト 3px、0 で非表示）専用枠が 100% に到達するとバーが警告色（`[color] claude_scoped_bar_warn`、デフォルト赤）に変わり、アカウント別に警告音・Toast 通知を発する（100% 未満へ戻るとリセット）。毎時 0 分に最新データを強制取得する。セッション数はアカウントごとに分けてカウントする。（各 `claude.exe` プロセスの `CLAUDE_CONFIG_DIR` 環境変数から判別）Sessions ラベルの左には直近の Usage API 取得時刻（`H:MM` 形式）を同色・同サイズで表示し、表示データの鮮度を確認できる。Usage API を取得できない間はプラン名の右に赤字で `Err` を表示する。未ログイン（OAuth トークン未取得）の間は取得可否に依らず `Logout` を表示し、`claude login` での再認証が必要。色による警告条件は後述の「Claude Code の警告条件」を参照
- **Claude Code nudge**：5h ウィンドウのリセット通過後にレートリミット消費が始まっていない間隙を検知したタイミング（起動直後を含む、ウィンドウごとに 1 回）で `claude.exe`（設定可能）を自動起動し、次のウィンドウでのレートリミット消費を促す機能。Usage 取得間隔も設定可能。Usage API 取得失敗（`Err` 表示）中も、把握済みの 5h リセット時刻を通過したら推定発火する（新ウィンドウ開始の有無は確認できないため。認証切れ由来の ERR では claude.exe の起動がトークン更新で ERR を治癒することがある）。`sysmeters.toml` の `[claude] nudge_enable = true`（メイン）/ `[claude_sub] nudge_enable = true`（サブ）でアカウント別に有効化。`nudge_cmd` は両アカウント共通で、サブ実行時は `CLAUDE_CONFIG_DIR` 環境変数に `claude_sub.config_dir` が一時設定された状態で起動される（Claude CLI に `--config-dir` コマンドオプションは存在せず、設定ディレクトリの上書きは環境変数経由のみ）。デフォルト OFF
- **トッププロセス表示**：CPU / GPU の面グラフ内に、使用率トップのプロセス名と使用率を `chrome 28%` の形式で表示する。同名 exe（Chrome の複数プロセス等）は合算し、`.exe` を除いた名前で表示する。CPU は `NtQuerySystemInformation` のスナップショット差分（0.9 秒間隔、システム全体を 100% とする尺度）、GPU は PDH の `\GPU Engine(*)\Utilization Percentage` の 3D/Compute 系エンジンを同名 exe・アダプタ・エンジン種別の単位で合算し、その最大値を採って求める。（約 2.7 秒間隔）同率（四捨五入した整数 % が同値）の間は直前に表示したプロセスを維持し、表示の入れ替わりを抑える。タスクトレイメニューの「トッププロセス表示」で ON/OFF を切り替えられ、OFF の間は収集自体を停止する。設定はレジストリに保持され、再起動後も維持される。デフォルト ON。大パーセンテージが `sysmeters.toml` の `[topproc] show_pct`（デフォルト 50%、0 で条件無効）未満の間と、トップの使用率が大パーセンテージ × `min_share_pct` / 100（デフォルト 50、0 で無効）に満たない間（負荷が特定プロセスに偏っていない状態）は表示を隠す。一度表示された後は各閾値の 8 割を下回るまで表示を維持する。（境界付近の点滅防止）表示条件を失った後も、最後に表示していたプロセス名と使用率を `[topproc] linger_sec` 秒（デフォルト 5 秒、0 で無効）だけ凍結して薄く表示し続け、一瞬のスパイクの見逃しを防ぐ。（残像表示。表示条件が再成立すると即座に実測値へ置き換わる）
- **コンパクト表示**：タスクトレイメニューの「コンパクト表示」で、グラフ・フォントを含む全体を 3/5 に縮小できる。設定はレジストリに保持され、再起動後も維持される
...
- 平均判定のサンプル数は `sysmeters.toml` の `[threshold]` の `avg_samples`（CPU/GPU、デフォルト 15 ≒ 13.5 秒）と `avg_samples_mem`（RAM/VRAM、デフォルト 7 = 14 秒）で調整可能（1 で瞬間値判定）
...
- 7d ウィンドウ開始（リセット）から `underuse_grace_hours`（デフォルト 48 時間）以上経過している
