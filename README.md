...
- CPU：全体使用率（面グラフ）、論理コア別使用率（縦バー）、温度を表示
...
- Claude Code 5h リセット通知：リセット通過を Toast と専用の通知音で知らせる（使用率が閾値以下なら除く）
- トッププロセス表示：CPU / GPU の面グラフ内に、使用率トップのプロセス名と使用率を表示
- 表示項目の切り替え：CPU、GPU、メモリ、ディスク、ネットワーク、Claude の各セクションを個別に ON/OFF
  - ディスクはドライブ単位でも切り替え可能
- コンパクト表示：グラフ・フォントを含む全体を 3/5 に縮小表示
- 更新確認：起動時に GitHub の最新リリースを確認し、新版があれば Toast とタスクトレイメニューで通知
...
- 横バー上の緑の縦線は均等消費ペースマーカー（リセットまでの残り時間で均等に消費した場合の理想位置）
- 5h バーの右側に、7d リセットまでに実行できる 5h ターンの残数（進行中のターンを含む）を表示
  - 表示位置は 7d リセット日時の月数字と右端を揃える
- 5h / 7d バーには直近 N 分間で増えた使用率を濃色で重ね塗りし、消費ペースを視覚化
  - 濃色は RAM の WSL オーバーレイと同色
- ヘッダ行の取得時刻の左に、直近（デフォルト 5 分以内）に消費がある間だけ濃色と同色のドットを明滅表示
- 7d バーの直下に、上位モデル（Fable 等）専用 7d 枠の消費率を示すグレーのミニバーを表示
  - 表示は Usage API が専用枠を返すアカウントのみ
- 専用枠が 100% に到達するとミニバーが警告色に変わり、アカウント別に警告音と Toast 通知を発する
  - 100% 未満へ戻ると警告状態をリセット
- 毎時 0 分に最新データを強制取得
...
### Claude Code 5h リセット通知

5h ウィンドウのリセット時刻を通過した瞬間に、アカウント別に通知する機能です。タスクトレイメニューの「5h リセット通知」で ON/OFF を切り替えます（デフォルトは ON）。ON のときは Toast と、警告音とは別の専用通知音 `agent_reset.wav` を同時に鳴らし、Toast 側の OS 標準通知音は鳴らしません。

直前の 5h 使用率が `reset_notify_min_pct`（デフォルト 1%）以下のリセットは、使っていないウィンドウのため通知しません。sysmeters の停止中に通過したリセットも起動時には通知しません。フルスクリーン抑制は警告と同じ条件で適用します。
...
- 閾値超過の Toast 通知では OS 標準通知音を鳴らさず、警告音に一本化（`alert_sound` 無効時は標準音を鳴らす）

### 通知の抑制

フルスクリーンアプリ（ゲーム・プレゼン等）の実行中は、Toast 通知と警告音を自動抑制します。

- ON/OFF はタスクトレイメニューの「フルスクリーン時は通知しない」で切り替え可能（デフォルト ON）
- 抑制中でも通したい警告は「常に警告通知を有効にする」で項目別に例外指定できる
  - 対象項目は CPU 使用率 / CPU 温度 / GPU 使用率 / GPU 温度 / ディスク温度
...
アンインストールしても、次のレジストリ設定とキャッシュディレクトリは残ります。不要であれば手動で削除してください。

- `HKEY_CURRENT_USER\Software\sysmeters`（タスクトレイメニューで切り替えた表示設定）
- `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run` の `sysmeters` 値
  - スタートアップ登録を ON にすると作成
- `%LOCALAPPDATA%\sysmeters`（Claude Code の使用状況キャッシュ）

## 使い方

Scoop でインストールしたときは、完了と同時に起動します。以降は `sysmeters` コマンドでも起動できます。

ZIP を展開したときは、展開先の `sysmeters.exe` を実行します。設定ファイルや警告音などの付属ファイルは実行ファイルと同じディレクトリから読み込むため、展開したファイルは分けずにまとめて置いてください。

タスクトレイ（通知領域）にアイコンを表示します。アイコンを左クリックすると、最小化中のウィンドウを復元して一時的に最前面へ表示します（フォーカスは移しません）。右クリックメニューでは、常に最前面に表示、コンパクト表示、トッププロセス表示、Toast 通知、5h リセット通知、Windows スタートアップ登録の切り替えと、表示項目の選択、設定ファイルとログファイルを開く操作、終了ができます。メニュー先頭にはバージョンを表示し、新しい版があればクリックで配布ページを開けます。
...
| `[claude]` | Usage API の取得間隔、Claude Code セクションの表示調整、nudge、5h リセット通知 |
...
- ディスクの温度と書き込み量は NVMe SSD からのみ取得し、それ以外のディスクでは表示しない
