...
- The blue vertical line on the 7d bar is the reach-limit marker (the approximate position beyond which 7d usage can no longer reach 100% even if 5h windows are run at 90% from now on; if the fill is left of the line, 100% is unreachable)
- The 5h-to-7d conversion ratio for the reach-limit marker is estimated automatically from the accumulated ratio of observed deltas (the line is hidden until a fifth of a 5h window of consumption has been observed, and on accounts whose Usage API does not return reset times)
...
A feature that automatically launches `claude.exe` at the moment it detects the gap after a 5h window reset where rate limit consumption has not yet started (including immediately after startup, normally once per gap), encouraging consumption in the next window. Default is off.
