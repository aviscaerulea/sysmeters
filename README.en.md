...
From the "表示項目" (Display Items) submenu in the tray icon's right-click menu, each section — CPU, GPU, memory, disk, network, and Claude — can be toggled on and off individually. Disks can additionally be toggled per drive. This also serves the purpose of minimizing the window by leaving only the categories you need. The figure below shows an example displaying only CPU and Claude.

![Example with display items narrowed down](docs/screenshot-selective.png)

When a sub account is enabled, the Claude section is displayed for both the main and sub accounts, stacked vertically. The figure below shows an example of the two-account display. The darker shading on the 5h / 7d bars indicates the usage increase over the last N minutes, visualizing the consumption pace.
...
Fixed drives present at startup (in drive-letter order from C:, up to 8 drives) are detected automatically. Virtual drives that report themselves as `DRIVE_FIXED`, such as Google Drive, are excluded based on whether a physical disk backs them. Display can be toggled per drive from "表示項目" (Display Items) in the tray menu; free space and S.M.A.R.T. warnings continue to be monitored even while a drive is hidden.
...
CPU figures come from snapshot deltas of `NtQuerySystemInformation` (0.9 second interval, on a scale where the whole system is 100%). GPU figures come from PDH's `\GPU Engine(*)\Utilization Percentage`, aggregating 3D/Compute engines per executable name, adapter, and engine type, then taking the maximum. (approximately 2.7 second interval) While values are tied (the rounded integer percentages are equal), the previously displayed process is retained to suppress flicker between entries.

The display can be toggled with "トッププロセス表示" (Top Process Display) in the tray menu; collection itself stops while it is off. The setting is persisted in the registry and survives restarts. Default is on.
...
- Directly below the 7d bar, a gray mini bar shows consumption of the dedicated 7d quota for higher-tier models such as Fable. (only for accounts whose Usage API returns a dedicated quota) When the dedicated quota reaches 100%, the bar changes to the warning color and an alert sound and Toast notification are raised per account (reset once it falls back below 100%)
- The latest data is force-fetched at the top of every hour
- Session counts are tallied separately per account. (determined from each `claude.exe` process's `CLAUDE_CONFIG_DIR` environment variable)
...
- While the Usage API cannot be fetched, `Err` is displayed in red to the right of the plan name. While logged out (no OAuth token), `Logout` is displayed regardless of fetch success, and re-authentication via `claude login` is required
...
- Toast notifications can be toggled from the tray icon's right-click menu (the setting is persisted in the registry)
- While a fullscreen application (games, presentations, etc.) is running, Toast notifications and alert sounds are automatically suppressed. This can be toggled with "フルスクリーン時は通知しない" (Do not notify in fullscreen) in the tray menu (default on)
- Warnings you want through even during fullscreen suppression can be excepted per item (CPU usage / CPU temperature / GPU usage / GPU temperature / disk temperature) via "常に警告通知を有効にする" (Always enable warning notifications) in the tray menu. Items turned on raise both Toast and alert sound even while suppressed. The three temperature items are on by default
- While fullscreen suppression is in effect, the window title reads `sysmeters (fullscreen:silent)`
- Windows startup registration can be toggled from the tray icon's right-click menu
...
The pace reference point is a sample from approximately 12 hours earlier; if unavailable, the oldest sample (once 30 minutes have elapsed) is used, and after the app resumes from being stopped, the last sample before the stop (the anchor) substitutes for it. No determination is made while the pace cannot be estimated (when the observed history spans less than 30 minutes, or when there has been no recent increase). The 7d history is saved to a file under `$TEMP` and, with the last sample before a stop as an anchor, retained for up to twice the window width (default 24h), so determination continues based on the effective pace from before the stop even across sysmeters or OS restarts. After a stop longer than 24h, the history is rebuilt and determination resumes in about 30 minutes.
...
An icon appears in the system tray (notification area). The right-click menu offers toggles for always-on-top, compact mode, top process display, and Toast notifications, along with display item selection, opening the configuration file, and exiting.

## Configuration

`sysmeters.toml` lets you customize appearance (background color, chart colors), warning thresholds, automatic process priority control, and more. When `sysmeters.local.toml` exists, its values override the settings in `sysmeters.toml`. Writing personal settings there keeps diffs small across updates.
...
- GPU and VRAM monitoring supports NVIDIA GPUs (NVML) only. The corresponding sections are not displayed in environments without NVML
...
- The Claude Code section requires being logged in via `claude login`. While logged out, `Logout` is displayed
...
- NVML: GPU, VRAM (dynamically loaded with `LoadLibrary`, so it runs without a GPU)
- WMI: CPU temperature
- WinHTTP: Claude Usage API, OAuth tokens
- WASAPI: alert sound playback
- toml11: configuration file
